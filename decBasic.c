...
#define MULACCLEN (MULOPLEN*2)    // 4 for 128 bit// accumulator length (ditto)
...
static void decFiniteMultiply(bcdnum *num, uByte *bcdacc,
  ...
  */
  ...
  static void MdecFiniteMultiply(bcdnum *num, 
	  const decFloat *dfl, const decFloat *dfr) {
	  uInt   bufl[4];           //2 for 64 bit left  coefficient (base-billion)
	  uInt   bufr[4];           //2 for 64 bit right coefficient (base-billion)
	  uInt   *ui, *uj;                 // work
	  uByte  *ub;                      // ..
	  uInt   uiwork;                   // for macros

#if DECUSE64
	  uLong  accl[4];          // lazy accumulator (base-billion+)
	  uLong  *pl;                      // work -> lazy accumulator
	  //uInt   acc[MULACCLEN];           // coefficent in base-billion ..
	  uLong acc[4];// it should be equal to MULACCLE but here 2
#else
	  uInt   acc[MULACCLEN * 2];         // accumulator in base-billion ..
#endif
	  uInt   *pa;
	...
	// step1
	 ...
	 //////////////////////////////////////////////////////////////
	  ...
	  int methodnumber = 4;   //1 = calculating 1X-9X in base billion METHOD 2
	  ...
	  for (int precomputei = 2; precomputei < 10; precomputei++) // 1-9X need to comment out BCD conversion part
	  {
	   ...
	   ///////////////////////////////////////////////////////////////////
		...
		// ui
		  ...
		  *pl = bufl[2] * precomputei;
		  *pl = bufl[3] * precomputei;
		  // 1 X to 9 X converter
	...
	#define mulsplit6 1000            // divisor
	  ...
	  case 2:
	  {
		  ...
		  for (int precomputei = 2; precomputei < 10; precomputei++) // 1-9X need to comment out BCD conversion part
		  {
			  ...
			  (tac)[5] += tacr[5];
			  if (tac[5] > 999) { tac[5] -= 1000; tac[6]++; }

			  (tac)[6] += tacr[6];
			  if (tac[6]>999) { tac[6] -= 1000; tac[7]++; }
			  (tac)[7] += tacr[7];
			  if (tac[7]>999) { tac[7] -= 1000; tac[8]++; }
			  (tac)[8] += tacr[8];
			  if (tac[8]>999) { tac[8] -= 1000; tac[9]++; }

			  (tac)[9] += tacr[9];
			  if (tac[9]>999) { tac[9] -= 1000; tac[10]++; }
			  (tac)[10] += tacr[10];
			  if (tac[10]>999) { tac[10] -= 1000; tac[11]++; }
			  (tac)[11] += tacr[11];
	  ...
	  case 3:
	  {
		  ...
		  for (int precomputei = 2; precomputei < 10; precomputei++) // 1-9X need to comment out BCD conversion part
		  {
			  ...
			  (bufl)[1] += bufl[1];
			  if (bufl[1] > 999999999) { bufl[1] -= 1000000000; bufl[2]++; }
			  (bufl)[2] += bufl[2];
			  if (bufl[2] > 999999999) { bufl[2] -= 1000000000; bufl[3]++; }
			  (bufl)[3] += bufl[3];
	  ...
	  case 4 :
	  {
		  ...
		  //Conversion from dpd to bcd for bcd additon  
